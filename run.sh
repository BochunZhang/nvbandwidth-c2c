...
#   {dir}/{name}.json     – nvbandwidth JSON result (bandwidth data only, clean)
#   {dir}/{name}.txt      – nsys status messages + any stderr from nvbandwidth
#   {dir}/{name}.nsys-rep – nsys CUDA/NVTX trace (generated by nsys profile)
#
# Why two separate invocations:
#   nsys profile writes its own status lines ("Collecting data...", "Generating ...")
#   to stdout, which pollutes the JSON output when both are redirected together.
#   Running them separately keeps the JSON file clean and parseable.
set -euo pipefail
...
run_test() {
    ...
    # Pass 1: bandwidth measurement – clean JSON, no nsys overhead.
    #   nvbandwidth with --json writes a single JSON blob to stdout at the end
    #   (JsonOutput::print() in json_output.cpp); stderr goes to .txt.
    ./nvbandwidth -t "$name" --cooldown 24 --json \
        > "$dir/${name}.json" \
        2> "$dir/${name}.err.txt"

    # Pass 2: nsys profiling – human-readable text output, full trace recorded.
    #   nsys status lines and nvbandwidth stdout/stderr all go to .txt.
    nsys profile --trace=cuda,nvtx \
        --output="$dir/${name}" \
        --force-overwrite true \
        ./nvbandwidth -t "$name" --cooldown 24 \
        >> "$dir/${name}.txt" 2>&1
